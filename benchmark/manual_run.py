...
class ScriptedAnswerClient:
    ...
    async def chat(self, messages: list[Message], tools: list[ToolSpec], policy: AgentPolicy) -> AsyncIterator[LLMEvent]:
        ...
        if self._turn.queries and not self._searched:
            self._searched = True
            async for event in self._search_events(self._turn.queries):
                yield event
            return
    ...
    async def _search_events(self, queries: list[str]) -> AsyncIterator[LLMEvent]:
        calls = [ToolCall(id=f"manual_{index}", name="search_and_read", arguments={"query": query}) for index, query in enumerate(queries)]
        for call in calls:
...
def print_result_line(question: Question, transcript: Transcript) -> None:
    searched = "search" if transcript.tool_exchanges else "no search"
