...
| 2026-09-05 | 03, 04 | The MCP Python SDK is at 2.x: the server class is `MCPServer` (was FastMCP) and the client is `mcp.client.client.Client`, which accepts a URL or an in-process server object. | Library moved between design and build; the in-process client made the tool server testable without sockets. |
