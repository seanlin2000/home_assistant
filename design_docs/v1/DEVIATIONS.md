...
| 2026-09-06 | Agent loop (`assistant_core/agent_loop.py`) | 04 | A completion with neither text nor a tool call is retried once with the same messages; the miss is recorded in `Transcript.empty_completion_retries`. | Gemma 4 E4B on Ollama returned empty completions on about one answer in seven because Ollama silently dropped slightly malformed tool calls; in production that is silence from the speaker. |
